...
class CommandRunner {
  ...
  /// If not null, this method is usedd to handle output. Useful if you want to
  ...
  ArgResults parse(List<String> input) {
    ...
    if (_commands.containsKey(input.first)) {
      ...
      throw ArgumentException(
          'The first word of input must be a command', null, input.first);
    }
    ...
    while (i < input.length) {
      if (input[i].startsWith('-')) {
        ...
        var option = results.command!.options.firstWhere(
            (option) => option.name == base || option.abbr == base, orElse: () {
          throw ArgumentException(
              'Unknown option ${input[i]}', results.command!.name, input[i]);
        });
